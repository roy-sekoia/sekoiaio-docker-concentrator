...
@pytest.mark.parametrize("template_name", ["template.j2", "template_tls.j2"])
def test_omrelp_action_enables_tcp_keepalive(template_name, monkeypatch):
    monkeypatch.setenv("RELP_OUTPUT", "1")
    rendered = render(template_name)
    assert 'type="omrelp"' in rendered
    action_block = rendered.split('type="omrelp"', 1)[1]
    assert 'tcp.keepalive="on"' in action_block, (
        f"{template_name}: omrelp action is missing tcp.keepalive=\"on\"."
    )
