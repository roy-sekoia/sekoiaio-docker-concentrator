...
#### Automatic recovery from a stuck intake

Under sustained load, the local TCP connection to an intake endpoint can silently
die (no RST received) while `rsyslogd` still considers it "established". TCP
`KeepAlive` is enabled on the output actions so the kernel can detect a dead peer
and let rsyslog's existing retry logic reconnect, instead of a write blocking
forever on the dead socket.

As defense-in-depth for other causes of a stuck output (e.g. known issues in
rsyslog's TLS output driver), the image also ships with a Docker `HEALTHCHECK`
that watches rsyslog's `impstats` and restarts `rsyslogd` automatically if an
intake's queue has a backlog but nothing has been forwarded for a few consecutive
intervals - combined with `restart: always`, this restores log forwarding without
manual intervention. This requires `restart: always` (or another restart policy) to
be set, otherwise the container will simply be reported as `unhealthy`.
